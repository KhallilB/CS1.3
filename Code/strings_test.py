...
class StringsTest(unittest.TestCase):
    ...
    def test_find_index_with_matching_patterns(self):
        ...
        assert find_index(
            'Ice Cream and Gelato are not synonymous', 'mous') == 35
        assert find_index('literally different recipes',
                          'different recipes') == 10
    ...
    def test_find_index_with_non_matching_patterns(self):
        ...
        assert find_index('kriss kross', 'ace') is None
        assert find_index('mes0thelioma', 'meso') is None
        assert find_index('truncatenate', 'trunk') is None
        assert find_index('Mr.Meeseeks', 'Look At Me!') is None
    ...
    def test_find_index_with_complex_patterns(self):
        ...
        # You'll need a lot more than this to test your algorithm's robustness
        assert find_index(
            '~the bridge is crossed so staaaaand and watch it buuuuuuuurnnnnn', 'the') == 1
    ...
    def test_find_all_indexes_with_matching_patterns(self):
        ...
        assert find_all_indexes('quality quidditch supplies', 'i') == [
            4, 10, 13, 23]
    ...
    def test_find_all_indexes_with_non_matching_patterns(self):
        ...
        assert find_all_indexes(
            'baby baby i feel crazy', 'i feel so sane this is a healthy relationship') == []
        assert find_all_indexes('dear scammer clairvoyant will i marry my celebrity crush',
    ...
    def test_find_all_indexes_with_complex_patterns(self):
        ...
        assert find_all_indexes('sandbox andromeda and androgyny', 'and') == [
            1, 8, 18, 22]
        assert find_all_indexes(
            'immunology biology otolaryngology etymology mythology', 'ology') == [5, 13, 28, 38, 48]
